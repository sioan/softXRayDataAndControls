...
class PrincetonDaqMultipleShot:
  def __init__(self, iNumShot, fExpDelay, fPostDelay, iOpenDelay, fBurstRate, bSimMode, bShutterMode):
  ...
  def configure(self):
    ...
    print "daq configure...",
    sys.stdout.flush()
    self.daq.configure(key=newkey, events=0)
    print " done."
...
def main():
  (llsOptions, lsRemainder) = getopt.getopt(sys.argv[1:], \
      'hm:e:r:p:o:s', \
      ['help', 'mshot=', 'expdelay=', 'rate=', 'post=', 'opendelay=', 'shutter', 'sim'] )
  ...
  print "Command line: NumShot %d ExpDelay %g PostDelay %g OpenDelay %d Rate %g SimMode %s Shutter %s" % \
    (iNumShot, fExpDelay, fPostDelay, iOpenDelay, fBurstRate, bSimMode, bShutterMode)
  try:
    princetonDaq = PrincetonDaqMultipleShot(iNumShot, fExpDelay, fPostDelay, iOpenDelay, fBurstRate, bSimMode, bShutterMode)
    iFail = princetonDaq.init()
    if iFail != 0:
    ...
    for cycle in range(cycles):
      # set your energy for this cycle
      ## calculate value
      ## write value to PV
      daq.begin(events=0,controls=['energyPV',value])
      princetonDaq.getMoreImages(iNumShot)
      n[cycle]=princetonDaq.daq.l3eventnum()
      daq.stop()
