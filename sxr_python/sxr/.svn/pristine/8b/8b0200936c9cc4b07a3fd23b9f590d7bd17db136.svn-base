...
def collect_projections(gasdetpv,camerapv,nprojections,gdet_threshold) :
    # Collect 200 horizontal projections while checking shots are
    ...
    for n_attempts in range(10) :
        ...
        gasdetpv.start_dropshot_counting(threshold=gdet_threshold)
...
def scan_mirror(mirrorpv, gratingpv, begin, end, stepsize, camerapv,gasdetpv,
    ...
    for pos in mirrorpos :
        ...
        projection_store = collect_projections(gasdetpv,camerapv,camera_proj,gdet_threshold)
        #print "Projection Store:",projection_store.shape
    ...
    #print sharpness
    #print sharpness_rms
    return (mirrorpos,gratingpos,sharpness,sharpness_rms)



def run_scan(begin,end,stepsize,gratingrule,photonE,
             gdet_threshold,camera_proj,
...
def fit_data(mirrorpos, sharpness, fitfunc) :
    
    optimalMirror,fittedSharpness = fitfunc(mirrorpos,sharpness)
    
    return optimalMirror,fittedSharpness
...
def fit_poly2(mirrorpos,sharpness) :
    """
    Fit mirror position vs sharpness to quadaratic to find optimal
    mirror position
    """

    # Fit data to quadratic
    coeff = np.polyfit(mirrorpos, sharpness, 2)
    ...
    if (coeff[0] > 0.0) :
        print "FOUND A MINIMA RATHER THAN MAXIMA !!"
        
    return optimalPos,np.polyval(coeff,mirrorpos)



def fit_poly4(mirrorpos,sharpness) :
    """
    Fit mirror position vs sharpness to 4-th order polynomial to find
...
def format_results(mirrorpos, gratingpos, sharpness, 
                   optimalmirror, optimalgrating):
    """
    Format collected data into nice formatted string
    """
    
    tableString =  "Mirror \t Grating \t Sharpness \n"
    tableString += "------ \t ------- \t --------- \n"
...
if __name__ == "__main__" :
    ...
    gratingrule = int(raw_input("Enter MONO grating rule (100/200):"))
    photonE = float(raw_input("Enter Photon Energy (eV):"))
    threshold=int(raw_input("Enter threshold for drop shot counting"))
    projections=int(raw_input("Enter number of projections to average"))
    ...
    mirrorpos,gratingpos,sharpness,sharpness_rms = \
        run_scan(begin=scan_begin,
                 ...
                 gdet_threshold=threshold,
                 camera_proj=projections)
        
    time_taken = time.time() - time_start
    print "Time take for scan (sec):",time_taken
    
    optimalMirrorPosition,fitted_sharpness = fit_data(mirrorpos,sharpness,
                                                      fit_poly2)
    optimalGratingPosition = find_grating_from_mirrorpos(mirrorpos, 
                                                         gratingpos,
                                                         optimalMirrorPosition
                                                         )
    fignum = plot_data(mirrorpos, sharpness,sharpness_rms, fitted_sharpness)
    scanresults =  format_results(mirrorpos, gratingpos, sharpness, 
                                  optimalMirrorPosition, 
