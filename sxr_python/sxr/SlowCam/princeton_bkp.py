...
class PrincetonDaqMultipleShot:
  def __init__(self, iNumShot, fExpDelay, fPostDelay, iOpenDelay, fBurstRate, bSimMode, bShutterMode,readout_time=0.0):
  ...
  # cpo changed this line
  def beginCycle(self,controls = None):
    print "BEGINCYCLE"
    if self.daq == None:
      return
    if self.bBeginCycle:
    ...
    try:
      ...
      print '---', controls
      if controls is None :
        self.daq.begin(events=0,controls=[])
      else:
...
class PrincetonMultipleShotInteractive:
  ...
  def run(self, iNumShot, fExpDelay, fPostDelay, iOpenDelay, fBurstRate, bSimMode, bShutterMode,readout_time=0):
    princetonDaq = PrincetonDaqMultipleShot(iNumShot, fExpDelay, fPostDelay, iOpenDelay, fBurstRate, bSimMode, bShutterMode,readout_time)
...
def main():
  (llsOptions, lsRemainder) = getopt.getopt(sys.argv[1:], \
      'hm:e:r:p:o:s', \
      ['help', 'mshot=', 'expdelay=', 'rate=', 'post=', 'opendelay=', 'shutter', 'sim','readout='] )
  ...
  try:
    PrincetonMultipleShotInteractive().run(iNumShot, fExpDelay, fPostDelay, iOpenDelay, fBurstRate, bSimMode, bShutterMode,readout_time)
  except:
