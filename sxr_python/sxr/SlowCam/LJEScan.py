...
class LJEScan : 
    ...
    def scan(self,energies, scan_result) :
        ...
        for energy,result in itertools.izip(energies,scan_result) :
            old_l3ventnum = self.andor_daq.daq.l3eventnum()
	    ...
	    # sleep after to let camera recover before next scan point
	    time.sleep(self.andor_scandelay)
...
if __name__ == "__main__" :    
    ...
    experiment = "sxrk4816"
    logdir =     "/reg/neh/operator/sxropr/" + experiment + "/scan_log"
    hostname = os.popen("hostname").read().strip()
    
    curFilePath = os.path.realpath(sys.argv[0])
    curFilename = os.path.basename(curFilePath)
    
    logfilename = "%s/%02d_%02d:%02d:%02d_%s:%s.log" % (logdir, curTime[2], curTime[3], curTime[4], curTime[5],
                                                        hostname,curFilename)
