...
class Daq:
  def __init__(self,host="sxr-control",platform=0):
    self._host=host
    ...
    self.__scanlogdir="/reg/neh/operator/sxropr/scanlog/"
    self.scanlogfile_f = self.__scanlogdir+"CURRENT_LOG_FILE"
  ...
  def set_filter(self,*k):
    """ enable data filtering:
        ...
        ***NO LONGER SUPPORTED***: set_filter(min,max); enable filtering between min and max on the monitor
        set_filter(det,mix,max); enable filtering using det
    """
    if len(k)==0:
      if self.__filter_det is not None:
        self.__filter_changed = True
        self.__filter_det = None
        self.__filter_min = None
        self.__filter_max = None
        self.__filter_string = None
  ...
  def connect(self):
    ...
    self.__daq.connect()
    sxrbeamline.sxrdaqconfig.create()
    return self.__daq

  def disconnect(self):
    #print "enter disconnect"
    sys.stdout.flush()
    sxrbeamline.sxrdaqconfig.unlock()
...
#         sys.stdout.flush()
  ...
  def configure(self,events=0,controls=[],monitors=[],key=None):
    self.connect()
    if (self.record is None):
      print "Record is what you set in GUI"
      if key == None:
        self.__daq.configure(events=events,controls=controls,monitors=monitors)
      else:
        self.__daq.configure(key=key,events=events,controls=controls,monitors=monitors)
    else:
      print "Record is: " , bool(self.record)
      if key == None:
        self.__daq.configure(record=bool(self.record),events=events,controls=controls,monitors=monitors)
      else:
        self.__daq.configure(key=key,record=bool(self.record),events=events,controls=controls,monitors=monitors)

  def begin(self,events=None,duration=None,controls=[],monitors=[]):
    if (self.__daq is None): self.configure(events=events,controls=controls,monitors=monitors)
    self.__running=True
  ...
  def calibcycle(self,events=None,controls=[],monitors=[]):
    ...
    #test
    #self.configure(events,controls=controls,monitors=monitors)
    ...
    try:
      ...
      for cycle in range(len(tPos)):
        ...
        if (self.settling_time != 0):
          ...
          pass
        if (config.TIMEIT>0):
          print "time needed for moving and settling %.3f" % (time.time()-t0move)
          pass
        self.__start_av(dets)
