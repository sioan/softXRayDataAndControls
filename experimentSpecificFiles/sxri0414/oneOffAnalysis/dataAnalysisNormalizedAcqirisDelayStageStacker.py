...
def projectionCalibration(x,y,calibrationFunction):
	...
	bestFit = dot(y,yEstimated)/dot(yEstimated,yEstimated)

	myUncertainty = std(y-yEstimated)/len(y)**0.5

	return bestFit,myUncertainty
...
if __name__ == '__main__':
	...
	mySlope = myCov[1,0]/myCov[0,0]
	#plot(gmdEdges[:-1],mySlope*gmdEdges[:-1],'r-')

	
	normalizedAcqiris = myDict['acqiris2']*myDict['GMD']/myPolyFunction(myDict['acqiris2'])
	#normalizedGMD = myDict['acqiris2']
	#plot(myDict['GMD'],normalizedAcqiris,'b.')
	#plot(gmdEdges[:-1],mySlope*gmdEdges[:-1],'r-')


	#projectionCalibration(x,y,interpolationFunctionMappinXtoY)
