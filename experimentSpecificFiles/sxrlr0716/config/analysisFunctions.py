...
def generic_acqiris_analyzer(detectorObject,thisEvent):
	...
	for i in arange(len(detectorObject[selfName](thisEvent)[0])):
		...
		initial_peak = argmax(abs(smoothed_wave))	#how to hardcode
		initial_height = smoothed_wave[initial_peak]
		peak_width = 10	############################## tunable parameter
		...
		try:
			#popt,pcov = curve_fit(peakFunction,x_small,y_small,p0=[1,initial_peak,initial_height])
		
			#fit_results['ch'+str(i+1)] = {"position":popt[1],'area':sum(y_small),'position_var':pcov[1,1],'amplitude_var':pcov[2,2]}
			fit_results['area']=sum(y_small)

		except (RuntimeError,TypeError) as e:
			#fit_results = None
			fit_results['ch'+str(i)] = {"position":-999999.0,'area':-999999.0,'position_var':-999999.0,'amplitude_var':-999999.0}

	#if(i==3 and 'Acq01' ==selfName):
	#	IPython.embed()
