...
class dls_viewer(CustomViewer):
	...
	def plot_subset(self, axes, x, y,z, style,bins,shot_by_shot,the_slope,median_truncation,state,modulation_spectroscopy,my_offset,Subset_Number):		
		...
		if my_hex_style_id not in self.my_sub_groups.keys():
			...
			self.my_sub_groups[my_hex_style_id]['offset'] = 0
		...
		chosen_id = list(self.my_sub_groups.keys())[int(Subset_Number)]
		...
		if(not (x_not_changed and y_not_changed)):
			if(len(z)!=0):
				#square brackets around tEdges is important
				self.my_sub_groups[my_hex_style_id]['y_data'] = vectorized_binned_statistic_dd(z,myValues,bins=[tEdges],statistic=t_func_wrapper)
				#normalization for displaying	
				self.my_sub_groups[my_hex_style_id]['y_data'][myStatistic]-=np.mean(self.my_sub_groups[my_hex_style_id]['y_data'][myStatistic])	
				self.my_sub_groups[my_hex_style_id]['y_data'][myStatistic]/=np.std(self.my_sub_groups[my_hex_style_id]['y_data'][myStatistic])	
		...
		try:	
			...
			#axes.plot(tEdges[:-1],myStats[myStatistic][::-1]+this_offset,c=style.color,marker='.',linewidth=2)
			axes.plot(tEdges[:-1],myStats[myStatistic][::-1]+self.my_sub_groups[my_hex_style_id]['offset'],c=style.color,marker='.',linewidth=2)
